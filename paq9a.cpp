...
HashTable<B>::~HashTable() {
  ...
  fprintf(stderr,"HashTable<%d> %1.4f%% full, %1.4f%% utilized of %d KiB\n",
...
LZP::~LZP() {
  ...
  for (int i=0; i<H; ++i)
    c+=(t[i]!=0);
  fprintf(stderr,"LZP hash table %1.4f%% full of %d KiB\n"
    "LZP buffer %1.4f%% full of %d KiB\n", 
    100.0*c/H, H>>8, pos<N?100.0*pos/N:100.0, N>>10);
  fprintf(stderr,"LZP %d literals, %d matches (%1.4f%% matched)\n",
    literals, matches, 
...
void paq9a(FILE* in, FILE* out, Mode mode) {
  if (!lzp && !predictor) {
    ...
    fprintf(stderr,"%8d KiB\b\b\b\b\b\b\b\b\b\b\b\b", allocated>>10);
  }
  if (mode==COMPRESS) {
...
void unstore(FILE* in, FILE* out) {
  ...
  int csize=get4(in);
  if (usize!=csize)
    fprintf(stderr,"Bad archive format: usize=%d csize=%d\n", usize, csize);
  static char* buf=0;
...
FILE* open_archive(const char* filename) {
  ...
  if (!in)
    fprintf(stderr,"Cannot find archive %s\n", filename), exit(1);
  if (!check_archive(in) || (MEM=getc(in))<'1' || MEM>'9') {
    fclose(in);
    fprintf(stderr,"%s: Not a paq9a archive\n", filename);
    exit(1);
  }
  return in;
}
...
void list(const char* archive) {
  ...
  while (true) {
    ...
    if (c) {   // start of new file?  Print previous file
      if (mode)
        fprintf(stderr,"%10.0f -> %10.0f %c %s\n", usum, csum, mode, filename);
      int len=0;
      filename[len++]=c;
    ...
    if (usize<0 || csize<0 || mode!='c' && mode!='s')
      fprintf(stderr,"Archive corrupted usize=%d csize=%d mode=%d at %ld\n",
  ...
  utotal+=usum;
  ctotal+=csum;
  fprintf(stderr,"%10.0f -> %10.0f total\n", utotal, ctotal);
  fclose(in);
}
...
int main(int argc, char** argv) {
  ...
  {
    fprintf(stderr,"paq9a archiver (C) 2007, Matt Mahoney\n"
  ...
  if (argv[1][0]=='a') {
    ...
    if (out) fprintf(stderr,"Cannot overwrite archive %s\n", argv[2]), exit(1);
    out=fopen("/dev/null", "wb");// devnull instead of arv[2] so as to not write
    if (!out) fprintf(stderr,"Cannot create archive %s\n", argv[2]), exit(1);
    fprintf(out, "pQ9%c", 1);
    int i=3;
  ...
  // Extract from archive
    ...
    extract(argc, argv);
    fprintf(stderr,"%1.2f sec\n", double(clock()-start)/CLOCKS_PER_SEC);
  }

  // Report statistics
  delete predictor;
  delete lzp;
  fprintf(stderr,"Used %d KiB memory\n", allocated>>10);
  return 0;
}
