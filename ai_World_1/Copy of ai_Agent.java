...
import java.awt.*; // for use of Point import java.util.*; // for use
of ArrayList

/**
 * ai_Agent uses A* pathfinding algorithm to find a path to Goal
 * and follow it. (can also use Depth First Search).
 * 
...
{   
    ...
    private final static boolean EXTRAINFO= true;    
    ...
    /*
      public ArrayList BFS(Point start, Point goal){            
      ...
      if ((myWorld.getObjectsAt(currentpoint.x,currentpoint.y,Goal.class)).size()>0)
      return path;
      else {
      ...
      for (int i=0;i<DIRECTIONS;i++){
      queue.add(new Point(currentpoint.x+xOffset(i),currentpoint.y+yOffset(i)));                    
      }
      }
